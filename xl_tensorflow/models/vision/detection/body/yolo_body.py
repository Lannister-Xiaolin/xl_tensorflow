...
from xl_tensorflow.models.vision.classification.efficientnet import EfficientNetB0, EfficientNetB1, EfficientNetB2, \
    ...
    mb_conv_block, BlockArgs, get_swish, \
    EfficientNetLiteB1, EfficientNetLiteB2, EfficientNetLiteB3, EfficientNetLiteB4
...
def spatial_pyramid_block(feature):
    ...
    feature_5 = layers.MaxPooling2D(pool_size=5, padding="same", name="spp_5")(feature_1)
    feature_9 = layers.MaxPooling2D(pool_size=9, padding="same", name="spp_5")(feature_1)
    feature_13 = layers.MaxPooling2D(pool_size=13, padding="same", name="spp_13")(feature_1)
...
def box_iou(b1, b2, method="iou", as_loss=False):
    ...
    if method == "iou":
        ...
        enclose_width = tf.maximum(0, enclose_xmax - enclose_xmin)
        enclose_height = tf.maximum(0, enclose_ymax - enclose_ymin)
        ...
        if method == "giou":
            ...
            diou_term = tf.math.divide_no_nan(tf.reduce_sum(tf.math.pow((b1_xy - b2_xy), 2), axis=-1),
                                              tf.math.pow((enclose_width * enclose_height), 2))
            diou = iou - diou_term
            return diou
            pass
        elif method == "ciou":
...
#
# class YoloLoss——old(tf.keras.losses.Loss):
#     """yolo损失函数
