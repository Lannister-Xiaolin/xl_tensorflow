...
def mb_conv_block(inputs, block_args, activation, drop_rate=None, prefix='',
    ...
    if has_se:
        ...
        if using_se_global_pooling:
            ...
            se_tensor = layers.AveragePooling2D(pool_size=(x.shape[1], x.shape[1]),
                                                name=prefix + "se_avg_pooling2d")(x)
...
def EfficientNet(width_coefficient,
    ...
    for idx, block_args in enumerate(blocks_args):
        ...
        x = mb_conv_block(x, block_args,
                          activation=activation,
                          drop_rate=drop_rate,
                          prefix='block{}a_'.format(idx + 1), using_se_global_pooling=using_se_global_pooling)
        block_num += 1
        if block_args.num_repeat > 1:
            ...
            for bidx in xrange(block_args.num_repeat - 1):
                ...
                block_num += 1
        if fpn_features:
            if (idx < len(blocks_args) - 1 and blocks_args[idx + 1].strides[0] == 2) or (idx == len(blocks_args) - 1):
                features[max(features.keys()) + 1] = x
    if fpn_features:
        return features
