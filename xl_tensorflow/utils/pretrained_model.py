...
def my_call_backs(model_name, monitor="val_loss", patience=5, reducelr=3):
    """回调函数列表，包括tensorboard, 学习率衰减, 提前终止，模型检测点"""
    if "win" in sys.platform:
        log_dir = os.path.join(os.getcwd(), r"\logs\{}".format(model_name))
        # print(log_dir)
    else:
    ...
    reducelr = ReduceLROnPlateau(monitor=monitor, factor=0.2, patience=reducelr, min_lr=0.00002)
