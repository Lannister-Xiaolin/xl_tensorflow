...
from ..utils.efficientdet_utils import activation_fn_no_feature
    ...
    def _build_class_net_layers(self, norm_activation):
        ...
        for i in range(self._num_convs):
            ...
            for level in range(self._min_level, self._max_level + 1):
                ...
                self._class_norm_activation[name] = [tf.keras.layers.BatchNormalization(
                    momentum=norm_activation.batch_norm_momentum,
                    epsilon=norm_activation.batch_norm_epsilon, name=name),
                    activation_fn_no_feature(act_type=norm_activation.activation, name=name + "_activation")
                ]
    ...
    def _build_box_net_layers(self, norm_activation):
        ...
        for i in range(self._num_convs):
            ...
            for level in range(self._min_level, self._max_level + 1):
                ...
                self._box_norm_activation[name] = [tf.keras.layers.BatchNormalization(
                    momentum=norm_activation.batch_norm_momentum,
                    epsilon=norm_activation.batch_norm_epsilon, name=name),
                    activation_fn_no_feature(act_type=norm_activation.activation, name=name + "_activation")
                ]
    ...
    def class_net(self, features, level, is_training):
        ...
        with self._class_name_scope:
            for i in range(self._num_convs):
                ...
                name = self._class_net_batch_norm_name(i, level)
                features = self._class_norm_activation[name][0](features)
                features = self._class_norm_activation[name][1](features)
    ...
    def box_net(self, features, level, is_training=None):
        ...
        with self._box_name_scope:
            for i in range(self._num_convs):
                ...
                # difference among different levels.
                name = self._box_net_batch_norm_name(i, level)
                features = self._box_norm_activation[name][0](features)
                features = self._box_norm_activation[name][1](features)
