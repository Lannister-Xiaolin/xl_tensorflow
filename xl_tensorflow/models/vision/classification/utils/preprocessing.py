...
def decode_and_center_crop(image_bytes: tf.Tensor,
                           ...
                           crop_padding: int = CROP_PADDING, no_crop_padding=True) -> tf.Tensor:
...
def decode_crop_and_flip(image_bytes: tf.Tensor,
                         min_object_covered: float = 0.7,
                         aspect_ratio_range: tuple = (0.4, 2.5),
                         area_range: tuple = (0.7, 1.0),
                         max_attempts: int = 10) -> tf.Tensor:
...
def preprocess_for_train(image_bytes: tf.Tensor,
    ...
    Returns:
      A preprocessed and normalized image `Tensor`.
    """
    images = decode_crop_and_flip(image_bytes=image_bytes, min_object_covered=min_object_covered,
                                  aspect_ratio_range=list(aspect_ratio_range),
                                  area_range=list(area_range),
                                  max_attempts=max_attempts )
